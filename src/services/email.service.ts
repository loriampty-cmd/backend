...
/**
 * Shared send helper — wraps resend.emails.send and normalizes error handling
 * so every function below doesn't have to duplicate the same try/catch logic.
 */
...
${emailConfig.appName} Team
  ...
  try {
    ...
    console.log(`✅ KYC approval email sent to ${email}: ${data?.id}`);
  } catch (err) {
    console.error(`❌ Failed to send KYC approval email to ${email}:`, err);
  }
}
...
${emailConfig.appName} Team
  ...
  try {
    ...
    console.log(`✅ KYC rejection email sent to ${email}: ${data?.id}`);
  } catch (err) {
    console.error(`❌ Failed to send KYC rejection email to ${email}:`, err);
  }
}
